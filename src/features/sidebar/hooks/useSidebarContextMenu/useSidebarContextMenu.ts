...
// The sidebar row a menu is open for.
export type SidebarMenuTarget = { path: string; kind: SidebarItemKind };

// Context-menu state for the sidebar: visibility, the targeted row, and positioning. Closes on
// an outside press or Escape, and clamps itself inside the viewport. Mirrors the directory's
// useContextMenu but carries a sidebar item (path + kind) instead of an entry id + EntryKind.
export const useSidebarContextMenu = () => {
  ...
  const [target, setTarget] = useState<SidebarMenuTarget | null>(null);
  ...
  // Open the menu at a screen position for a given row.
  const openAt = (x: number, y: number, nextTarget: SidebarMenuTarget) => {
  ...
  return { ref, visible, target, openAt, close };
};
